...
install:
	...
	run_pnpm --filter @ai-kiwi/cli build
	@mkdir -p "$(BINDIR)"
	@printf '%s\n' \
		'#!/usr/bin/env sh' \
		'set -eu' \
		...
		'  run_pnpm --dir "$$REPO_ROOT" --filter @ai-kiwi/cli build >/dev/null' \
		'fi' \
...
build:
	@if command -v pnpm >/dev/null 2>&1; then pnpm --filter @ai-kiwi/cli build; else corepack pnpm --filter @ai-kiwi/cli build; fi
