...
function matches(event: AuditEventLine, params: { runId: string; phase?: string; sinceMs: number | null }): boolean {
  ...
  if (params.sinceMs !== null && Date.parse(event.timestamp) < params.sinceMs) return false;
  if (!params.phase) return true;
...
function readNewLines(target: string, offset: number): { lines: string[]; offset: number } {
  if (!existsSync(target)) return { lines: [], offset };
...
export async function runTail(
  ...
  const printNew = (): void => {
    ...
    for (const line of result.lines) {
      ...
      if (opts.phase) matchInput.phase = opts.phase;
