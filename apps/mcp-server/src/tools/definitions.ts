...
const runIdProperty = { type: "string", description: "Kiwi run id. Omit to use the active repo run." } as const;
...
const TOOL_SPECS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        runId: runIdProperty,
        message: { type: "string", minLength: 1, description: "Human feedback or requested adjustment." },
        author: { type: "string", minLength: 1, description: "Optional human/operator identity." },
        targetStepId: { type: "string", description: "Optional step id the feedback targets." },
        targetAttemptId: { type: "string", description: "Optional attempt id the feedback targets." },
        ...WORKSPACE_PROPERTIES,
      },
      required: ["message"],
      additionalProperties: false,
...
const TOOL_ANNOTATIONS: Record<ToolName, ToolAnnotations> = {
  ...
  kiwi_feedback: {
    title: "Record kiwi feedback and replan",
    readOnlyHint: false,
    destructiveHint: false,
    idempotentHint: false,
    openWorldHint: false,
